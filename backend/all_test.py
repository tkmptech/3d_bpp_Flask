...
import json
import copy
#First placement as baseline, no need for this step, juniors don't need baseline
#Call Ptr for each order to get the order
#Don't consider if there is something in the bin
#Output value is the subscript number
#Input: [batch_size, input_size_item, seq_len_items]
#Output: probs: [seq_len_items*, batch_size, seq_len_items], idxs: [seq_len_items*, batch_size]
'''1: Kit code (not set to 0)
2-5: Length, width, height, weight
6-9: Is the first orientation established (0-1), is the first load-bearing established (0-1), load-bearing (0-5), stacking (0-5)
10-13: Is the second orientation established (0-1), is the second load-bearing established (0-1), load-bearing (0-5), stacking (0-5)
14-17: Is the third orientation established (0-1), is the third load-bearing established (0-1), load-bearing ( 0-5), stacking (0-5)
18-21: Is the 4th orientation established (0-1), Is the 4th load-bearing established (0-1), Load-bearing (0-5), stacking (0-5)
12-25: Is the 5th orientation established (0-1), Is the 5th load-bearing established (0-1), Load-bearing (0-5), stacking (0-5)
26-29: Is the 6th orientation established (0-1), Is the 6th load-bearing established (0-1), Load-bearing (0-5), stacking (0-5)
'''
# !/usr/bin/env python
# coding: utf-8
...
def all_run(json_data):
    ...
    # print(json_data)
    init_data(json_data)
    print('Number of bins used', len(Task.Used_bins))
    ...
    for packer in Task.Packers:
        ...
        model.load_state_dict(torch.load('pointer.pkl',map_location=device))
        model.eval()
        ...
        for i in range(num_bin):
            #If the box is full of these orders break run the next order, otherwise go up and down the next box
    ...
    for bin in Task.Used_bins:
        ...
        print('Cargo box utilization rate')
    ...
    Pathout = os.path.join(app_path(), 'data/output.json')
    Pathout3d = os.path.join(app_path(), 'data/output_3d.json')
