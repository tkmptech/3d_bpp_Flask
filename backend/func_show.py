...
def item_show(bin_x, bin_y, bin_z, items):
    ...
    plt.title("3D Bin Packing Show")
    plt.show()

def file(file_path):
    with open(file_path, 'r', encoding='utf-8') as fp:
        json_data = json.load(fp)
    binlist = json_data['Box']
    for jsbin in binlist:
        bin_width = jsbin['Cargo box length']
        bin_height = jsbin['Cargo box width']
        bin_depth = jsbin['Cargo box height']
        bin_item = jsbin['Cargo box cargo']
        item_show(bin_width, bin_depth, bin_height, bin_item)

# Example usage
if __name__ == '__main__':
    Path20 = os.path.join(app_path(), 'data/output_3d.json')
    file(Path20)
